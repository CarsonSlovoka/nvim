...
local function install_nvim_dap()
  ...
  dap.adapters.notepad = { -- 名稱也可以大小寫混合
    type = 'executable',
    command = "notepad"
  }
  dap.configurations.test_text = { -- 記得看的不是附檔名，而是filetype
    -- 如果只有一個執行的時候就不會看到有選單
    -- 如果有多個時會讓你選擇要用哪一個來執行
    {
      type = "notepad", -- 定義的type一定要在dap.adapters之中可以找到，不然會報錯
      request = 'launch',
  ...
  dap.adapters.ttx = {
    type = 'executable',
    command = "echo", -- 找一個不重要的指令, 為了通過dap而已 -- 這個工具在 Linux / macOS / Windows shell 都有
  }

  dap.configurations.opentype = {
    {
